...
class Rental
  attr_reader :id, :car_id, :start_date, :end_date, :distance, :actions
  ...
  def create_actions
    @actions << Action.new('driver', 'debit', total_price).action_result
    @actions << Action.new('owner', 'credit', (total_price * 0.7).to_i).action_result
    ...
    @actions << Action.new('drivy', 'credit', commission('drivy')).action_result
  end

  # Method that transforms the result into the required format
  def result
    { id: @id, actions: create_actions }
  end
end
